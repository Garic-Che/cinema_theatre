    ...
    async def connect(self) -> None:
        while True:
            try:
                ...
                period = settings.websocket_queue_reconnection_period
                await self.logger.error(
                    f"Error connecting to websocket queue. Reconnecting in {period} seconds"
                )
                await asyncio.sleep(period)
            else:
                await self.logger.info("Connected to websocket queue")
                break
