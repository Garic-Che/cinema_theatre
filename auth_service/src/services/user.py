...
class UserService:
    ...
    async def get_user_profile(self, request: Request) -> User | None:
    ...
    async def get_user_profile_by_id(self, user_id: str) -> User | None:
        """Возвращает данные профиля пользователя."""
        user = await self.auth_engine.get_user_by_id(self.session, user_id)
        if not user:
            raise HTTPException(
    ...
    async def exit_user(self, response: Response) -> str | None:
        """Выход пользователя."""
