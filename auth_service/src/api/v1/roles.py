...
from utils.authtorize_helpers import check_privilege, check_privilege_or_internal_auth
...
async def create_role(
    ...
    role = await service.create_role(
        name=role_data.name, privilege_ids=role_data.privilege_ids
    )
    return RoleResponse(
        id=role.id,
        name=role.name,
...
async def delete_role(
    request: Request,
    role_id_data: CommonID,
    service: RoleService = Depends(get_role_service),
) -> Message:
    msg = await service.delete_role(role_id_data.id)
    if not msg:
        raise HTTPException(
...
async def change_role(
    request: Request,
    role_data: RoleWithID,
    service: RoleService = Depends(get_role_service),
) -> RoleWithID:
    retval = await service.change_role(
        role_data.id, role_data.name, role_data.privilege_ids
    )
    if not retval:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail="bad request"
...
@check_privilege_or_internal_auth("role/")
async def all_roles(
    request: Request,
    service: RoleService = Depends(get_role_service),
) -> list[RoleWithID]:
...
@check_privilege_or_internal_auth("role/assign")
async def assign_role(
    ...
    msg = await service.assign_role(role_id_data.id, user_id)
    if not msg:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail="bad request"
        )
...
@check_privilege_or_internal_auth("role/revoke")
async def revoke_role(
    ...
    msg = await service.revoke_role(role_id_data.id, user_id)
    if not msg:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail="bad request"
        )
