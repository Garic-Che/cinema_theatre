...
async def register_user(
    ...
    user_id = await service.register_user(
        user_data.login, user_data.password, user_data.email or None
    )
...
async def enter_user(
    ...
    msg = await service.enter_user(
        request, response, user_data.login, user_data.password
    )
    if not msg:
        raise HTTPException(
...
@router.get(
    "/email/{user_id}",
    response_model=Message,
    summary="Получение email пользователя",
)
@internal_auth_required
async def user_email(
    ...
    email = await service.get_user_email(user_id)
    if not email:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail="User not found"
        )

    return Message(detail=email)
...
async def change_login(
    ...
    validate_user(login_data.new_login)

    msg = await service.change_login(payload["user_id"], login_data.new_login)
    if not msg:
        raise HTTPException(
