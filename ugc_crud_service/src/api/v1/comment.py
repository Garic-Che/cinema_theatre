...
from schemas.model import CommentPostDTO, CommentUpdateDTO, Notification
...
async def post_comments(
    ...
    comments = [
        Comment(**raw.model_dump())
        for raw in request
        if raw.user_id == user_id
    ]
    comments = await service.insert(comments)
    for comment in comments:
        ...
        await notification_service.send_notification(
            Notification(
                to_id=to_id,
                content_key=f"comment/{comment.content_id}",
                content_data=comment.user_id,
            )
        )
    return comments
